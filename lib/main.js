...
function incrcovmat( out, means ) {
	...
	if ( isPositiveInteger( out ) ) {
		...
		throw new TypeError( format( '1HBHz', out ) );
	}
	...
	if ( arguments.length > 1 ) {
		if ( !isVectorLike( means ) ) {
			throw new TypeError( format( '1HBI0', means ) );
		}
		if ( numel( means.shape ) !== order ) {
			throw new Error( format( '1HB9V,I1', order, numel( means.shape ) ) );
		}
	...
	*/
	function accumulator1( v ) {
		...
		if ( !isVectorLike( v ) ) {
			throw new TypeError( format( '1HBI2', v ) );
		}
		if ( v.shape[ 0 ] !== order ) {
			throw new Error( format( '1HB9W,I3', order, v.shape[ 0 ] ) );
		}
		n = N;
		N += 1;
		r = n / N;
